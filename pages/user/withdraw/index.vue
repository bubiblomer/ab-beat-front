<template>
    <div>
        <Breadcrumb title="Withdraw"></Breadcrumb>
        ...
        <section class="contact-section-two" >
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-12">
                        <div class="section-title p-4 justify-content-between media">
                            ...
                            <NuxtLink to="/user/withdrawLog" class="btn btn-warning text-white"><i class="fa fa-list"></i> Withdraw Log</NuxtLink>
                        </div>
                ...
                <div v-if="preview == 0" class="row justify-content-center mt-3">
                    <div class="col-md-4" v-for="(gateway, index) in withdrawMethodList">
                        <div class="card mb-4 darcula-bg">
                            <div class="card-body text-center">
                                ...
                                <ul class="list-group my-2">
                                    <li class="list-group-item bg-dark">Withdraw Limit: <span class="text-right">{{gateway.min_amount}} - {{gateway.max_amount}} {{basic.currency}}</span>
                                    </li>
                                    <li class="list-group-item bg-dark"> Charge : <span class="text-right">{{gateway.fixed_charge}} {{basic.currency}}   {{(0 < gateway.percent_charge) ? `+ ${gateway.percent_charge} %` : ''}} </span>
                                    </li>
                                </ul>
                                <button type="button" class="btn  btn-warning text-white  btn-block" @click="showModal(gateway.id)">
                                    Select
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="card darcula-bg mb-4">
                            ...
                            <div class="card-body">
                                <ul class="list-group font-weight-bold text-center">
                                    ...
                                    <li class="list-group-item bg-dark">{{logs.available_balance}}</li>
                                    <li class="list-group-item bg-dark">
                                        <NuxtLink :to="{name: 'user-withdraw-confirmation-pay', params:{pay: logs.track }}"
                                                  class="btn btn-lg btn-warning text-white btn-block">Pay Now
                                        </NuxtLink>
                                    </li>
                                </ul>
                    ...
                    <div class="modal-body text-left">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <input v-model="form.amount" type="text" class="form-control form-control-lg"
                                       placeholder="0.00">
                            ...
                            <small class="form-text text-danger" v-if="errors.gateway_id">{{errors.gateway_id[0]}}
                            </small>
                        </div>


                        <button class="btn btn-lg btn-warning text-white btn-block" type="button" @click.prevent="submitModal">Yes</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        head () {
            return {
                title: 'heeraexchange.in  | Withdraw',
            }
        },
        data(){
            return {
        ...
        async asyncData({$axios, store}){
            ...
            await $axios.get('/withdraw')
                .then(res => {
                    store.dispatch("setWithdrawMethodListData", res.data)
                })
                .catch(err => {
                });
        ...
        computed: {
            ...mapState(['withdrawMethodList','basic']),
        },
        methods:{
            showModal(gatewayId) {
            ...
            async submitModal() {
                var _this = this;
                await this.$axios.$post('/withdrawCalc', this.form)
                    .then(data => {
